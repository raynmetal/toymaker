...
#ifndef TOYMAKERENGINE_SPATIALQUERYOCTREE_H
#define TOYMAKERENGINE_SPATIALQUERYOCTREE_H
...
namespace ToyMaker {
    ...
    public:
         ...
         *
         * This is used when, after the removal of an object, octree nodes higher up in the tree are no longer required.  A node that was
         * previously a descendant node is promoted to the root node of the octree, and all addresses are changed accordingly.
         *
         ...
         *
         * @param interactionMask the interaction layers in which entities are being searched for
         *
    ...
    private:
        ...
        /**
         * @brief Add up interaction layer set of child octants and entities to make this
         * octant's new interaction set
         *
         */
    ...
    public:
        ...
        /**
         * @brief Gets a list of layers entities under this octant belong to, for which they will participate
         * in spatial query or collision events
         *
         */
        inline InteractionLayerMask getInteractionLayers() const { return mWorldBounds.getInteractionLayers(); }
    };
     ...
     *
     * The octree is essentially a node representing a cuboidal region that can be subdivided into equally-proportioned smaller cuboidal regions
     * recursively.  Each node of the octree maintains a list entities contained by its region.
     *
     * When given a query in the form of some geometry (currently only AABBs and Rays), where entities overlapping or contained by that geometry
     * are desired, the octree speeds up the query by limiting its search to only those entities whose nodes overlap the query geometry.
     *
     * Octrees presume the finiteness of geometry contained by them.  An octree can grow in size or shrink so long as the region it encloses does not
     * become infinite, nor fall below some threshold granularity.
     *
     * @see OctreeNode
     *
     */
    ...
    public:
         ...
         *
         * Specified in order to keep nodes more-or-less cuboidal, and preventing them from being too flat or line-like (and
         * thereby causing the octree to have to expand or shrink too often).
         *
         */
         ...
         * @brief Retrieves all entities in this octant and its descendants.
         *
         * @param layerMask Layers in which entities are being searched for
         *
         * @return std::vector<std::pair<EntityID, AxisAlignedBounds>> A list of entities (their IDs and AABBs) known by the octree.
         *
         */
        inline std::vector<std::pair<EntityID, AxisAlignedBounds>> findAllMemberEntities(InteractionLayerMask layerMask=std::numeric_limits<InteractionLayerMask>::max()) const {
            return mRootNode->findAllMemberEntities(layerMask);
        }
